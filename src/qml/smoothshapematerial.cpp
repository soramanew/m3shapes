...
const QSGGeometry::AttributeSet& smoothShapeAttributes() {
    ...
    static const QSGGeometry::AttributeSet set = { 3, sizeof(SmoothVertex),
        attributes };
    return set;
}
...
// Feather width in DEVICE pixels. ~1px gives a crisp edge; the value is
// constant on screen at any scale because the shader expands after the combined
// matrix.
...
public:
    SmoothShapeShader() {
        setShaderFileName(VertexStage,
            QStringLiteral(":/m3shapes/shaders/smoothshape.vert.qsb"));
    ...
    bool updateUniformData(RenderState& state, QSGMaterial* /*newMaterial*/,
        ...
        // Refresh viewport-derived pixel size every draw: the viewport (and
        // thus the device-pixel mapping) can change without isMatrixDirty()
        // tripping.
        const QRect vp = state.viewportRect();
        const float vpW =
            vp.width() > 0 ? static_cast<float>(vp.width()) : 1.0f;
        const float vpH =
            vp.height() > 0 ? static_cast<float>(vp.height()) : 1.0f;
        const float pixelToNdc[2] = { 2.0f / vpW, 2.0f / vpH };
        std::memcpy(data + k_uboPixelOffset, pixelToNdc, sizeof(pixelToNdc));
        std::memcpy(
            data + k_uboAaWidthOffset, &k_aaWidthPx, sizeof(k_aaWidthPx));
